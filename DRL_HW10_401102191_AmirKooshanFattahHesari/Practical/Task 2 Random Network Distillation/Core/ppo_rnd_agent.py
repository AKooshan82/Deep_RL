...
class Brain:
    ...
    def calculate_int_rewards(self, next_obs, batch=True):
        ...
        predictor_encoded_features = self.predictor_model(norm_obs)
        target_encoded_features = self.target_model(norm_obs)
        # Compute squared error (MSE) between predicted and target features
        mse = (predictor_encoded_features - target_encoded_features).pow(2)
        # Take mean over feature dimension (dim=1)
      
        int_reward = mse.mean(1)  # Replace this with prediction error computation

        return int_reward.detach().cpu().numpy() # → np.array
    ...
    def calculate_rnd_loss(self, obs):
        ...
        encoded_target_features = self.target_model(obs)
        encoded_predictor_features = self.predictor_model(obs)
        ...
        final_loss = (mask * loss).sum() / torch.max(mask.sum(), torch.Tensor([1]).to(self.device))
        return final_loss
